...
Chromium 内核浏览器扩展，自动识别验证码并登录南京大学统一身份认证平台。

> ⚠️ **隐私声明**：本扩展完全离线运行，验证码识别在本地完成，**不会上传任何用户数据**（包括账号、密码、验证码），无网络请求，无隐私泄露风险。
...
|--------|------|------|
| Chrome | ✅ | 主要支持目标 |
| Edge | ✅ | 基于 Chromium，直接兼容 |
| Brave / Opera / Vivaldi 等 | ✅ | Chromium 内核，直接兼容 |
| Firefox | ❌ | 不支持 Offscreen API，MV3 CSP 不兼容 `wasm-unsafe-eval` |
| Safari | ❌ | 不支持 Offscreen API，MV3 支持有限 |

本扩展依赖 Chrome MV3 专属的 **Offscreen Document API** 运行 ONNX WASM 推理，Firefox 和 Safari 目前不支持此 API，因此无法兼容。
...
📖 视频教程：[Bilibili - Chrome 扩展程序安装教程](https://www.bilibili.com/video/BV1JE411G7kT/)
...
- [ ] **支持 Firefox** — 使用 `background.page` 替代 Offscreen Document 运行 ONNX 推理，运行时检测浏览器切换路径，适配 Firefox MV3 CSP 策略
